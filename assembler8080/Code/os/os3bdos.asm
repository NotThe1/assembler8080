...
;*****************************************************************
;*****************************************************************
; "C:\Users\admin\Dropbox\Resources\CPM\CPM_source\2.0\cpm20_code\os3bdos.asm"
...
sta$ret:
	;store the A register to StatusBDOSReturn
...
print:
	...
	ldax b
	 ...
	 jmp print
;
read:	;read to ParamsDE address (max length, current length, buffer)
	lda column
...
UserStack:	ds	2	;entry stack pointer
...
getdm:
	...
	lhld ParamsDE ;base address of file control block
	lxi d,dskmap
	 dad d ;HL =.diskmap
...
copy$dir:
	...
	lhld ParamsDE ;HL = source for data
	dad b
...
close:
	...
	;merge the disk map at ParamsDE with that at buff(dptr)
	lxi b,dskmap
	 call getdptra
	dad b
...
diskwrite:	;(may enter here from seqdiskwrite above)
	...
	lda seqio
	 ora a
	 jz diskwr3 ;skip next extent open op
...
setrandom:
	...
	call compute$rr ;DE=ParamsDE, A=cy, BC=mmmm eeee errr rrrr
	lxi h,ranrec
	 dad d ;HL = .fcb(ranrec)
	mov m,c
	 inx h
...
;
func26:
	;set the subsequent dma address to ParamsDE
	lhld ParamsDE
	 shld dmaad ;dmaad = ParamsDE
        jmp setdata ;to data dma address
	;ret ;jmp goback
;
func27:
...
retmon:
	...
	lhld StatusBDOSReturn
	 mov a,l
	 mov b,h ;BA = HL = StatusBDOSReturn
	ret
;
;	data areas
;
