...
;		ORG	RAM + 0100H
TPA:		EQU	0100H			; ,Start of transient program area where programs are loaded.
;-------------------------------------------------------------------------------------
...
control6:	DW	control2				; read Only

pauseLimit:	DB	23			; read Only - L010C
controlW:	DB	000H				; read & write  ****
control7:	DB	0FFH				; read Only
...
; this must be the start of a file ie 1A
processEntry:
	CALL	loadBuffHeader			;  else move 1C/18 bytes of data  into hdrBuffer
	CALL	setSubFileName			; move file name to wFileName 
...
noComTail:				; L0681:
	...
	LDA  control2				; else load contents of control2 ie 10
	ORA  A
noComTail1:
	LXI  H,001ABH
	LXI  B,00480H
...
noComTail2:
	LXI  D,0014CH
	JMP  sendStringNL00NLERR				;005A1H
...
inControl2Not:				; L06C4:
	...
	POP  H
	ORA  A
	JNZ  inControl2Not1				; if its not 0, jump, zero out and return
...
; set target drive's blockSizeInK
	...
	POP	PSW				; restore current drive and flags from compare
...
makeTable1819B:
	...
	INR	A
	DCR	A
	MOV	C,A				; return manipulated data
	XTHL				; put COUNT on stack, get pre manipulated data
	MOV	A,H				;     into Acc
	...
	XTHL				; put PSW in HL
	SHLD	01792H				; store PSW for later
	POP	PSW				; pop JUNK  (808A) into PSW
	POP	H				; restore new pointer
	MOV	A,C				; get origial LSB
	XRA	D
	MOV	C,A				; put XOR'd data in C
	PUSH	H				; put pointer on stack
	PUSH	PSW				; save XOR'd result
	LHLD	01792H				; point at first saved PSW
	XTHL
	SHLD	01792H				; replace with new PSW
	POP  PSW
	POP  H
	XRA  E
makeTable1819C: 
...
L090C:
	...
	MVI	H,ZERO				; set MSB to 00
	LXI	D,tblMemNeeded
	DAD	D
	MOV	A,M				; get min pages for this type
	CALL	enoughMemory				; number of pages in Acc
	LXI	H,memReqType
	MOV	M,A				; save required pages
	MOV	C,A
	INX	H
	LDA	workingMemory				; get memory available
	MOV	M,A				; and save
	INR	B				; see if we are working with current drive
	JNZ	0095FH				; skip if nothing there
	LDA	control4
	ORA	A
	CZ	inControl2Not
...
doFileInfo2:
	CALL	doCRC				; put the CRC info into outBuffer1
doFileInfo3:
	LXI	D,OutBuffer1				; point at the text data
	JMP	doFileInfo99				; send it to the screen
...
doFileInfo10:
	...
	LXI	D,txtULine
doFileInfo99:
	MVI	M,000H				; terminate message string with Zero
	JMP	sendStringNL00				; Send new line, then message pointed at in DE


sendCharOut:
	CPI	BELL
	JNZ	sendCharOut1 				; skip if BELL
	LXI	H,control7
...
sendCharOut2:
	...
	JZ	sendCharOut3  	 				; jump it indeed it was a SOH
	MVI	C,vConsoleOutput
	CALL	BDOSE				; send char in E to Screen
	POP	D
	RET
;
sendCharOut3:
	LXI	D,0014CH				; point at ?????
sendCharOut4:
	LDAX	D				; get char pointed at by DE
	CPI	SPACE
	JZ	sendCharOut2				; send char from Acc
	CALL	sendCharOut				; else go thru all the tests
	INX	D
	JMP	sendCharOut4				; else loop
;
sendStringNL00NL:
	CALL	sendNL
sendString00NL:
...
isConsoleReady:
	CALL	checkConsoleIn
	JZ	isConsoleReady				; loop until console is ready
	PUSH	PSW
	LXI	D,015A5H				; point at CR lots of spaces CR
	CALL	sendString00				; send to console
	POP	PSW
	POP	D
	XRI	020H
	RNZ
	INR	A
	STA	pauseCount				; save pause count
	RET
;---------------
...
;---------------
doCRC:
	CALL	fillMemWith2Spaces				; move to next column
	XCHG				; put outBuffer1 pointer in DE
	LHLD	hdrCRC				; get CRC info from hdr
	XCHG				; DE has hdrCRC, HL=>outBuffer1
	PUSH	H				; save outBuffer1 pointer
	LHLD	sumCRC				; get sum
	DAD	D				; add hdrCRC
	SHLD	sumCRC				; and put it back
	POP	H				; retreive outBuffer1 pointer
doCRC1:
	CALL	doCRC2
	MOV  D,E
doCRC2:
	MOV	M,D				; put msb of hdrCRC into outBuffer1
	MOV	A,M				; get that value into Acc
	RLC
	RLC
	RLC
	RLC				;  swap nibbles 
	CALL	doCRC3
	MOV	A,D				; back to origial order of nibbles
doCRC3:
	ORI	0F0H				;
	DAA				; decimal adjust Acc
	CPI	060H
	SBI	01FH
	MOV	M,A				; put Ascii value into outBuffer1 
	INX	H				; point at next position
	RET
;---------------
