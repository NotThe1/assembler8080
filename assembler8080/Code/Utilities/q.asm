...
CKDONE:
	...
	JNZ		PRHEX		;	NO, LOOP
NOWASC:
	CALL	FENCE		;PRINT FENCE CHAR
	POP		H		;RESTORE ADDR
PRASC:
	MOV		A,M		;GET BYTE FOR ASCII
	CPI		20H		;SEE IF CONTROL CHAR
	JC		PRPER		;PRINT PERIOD IF SO
PRTABLE:
	CPI		7FH		;SEE IF PRINTABLE
	JC		PRNPER		;	YES
PRPER:
	MVI		A,2EH		;PRINT A PERIOD
PRNPER:
	CALL	CONOUT		;PRINT WHAT'S IN A
MORE:
	INX		H		;TO NEXT CHAR
...
TIMES16:
	...
	DAD		H		;      16
CKALPHA:
	CPI		41H		;SEE IF CHAR IS 
	JC		NUM		;	NUMERIC?
				;(NOT THOROUGH TEST)
	SUI		7		;OTHERWISE FUDGE
NUM:
	SUI		30H		;TO MAKE PRINTABLE
				;(SEE TEXT)
...
;
DUMPIT:
	CALL	PRADDR		;FULL DUMP: PRINT ADDR,
	CALL	PRLINE		;	THEN LINE
CKSTAT:
	CALL	STAT		;THEN CHECK FOR INTERRUPT
	JZ		EXIT		;	AND EXIT IF SO.
STOPAT0:
	MOV		A,H		;CHECK IF HL = 0
...
GETCHAR:
	PUSH	H		;GOT A CHAR, SAVE ADDR
	MVI		C,CONIN		;GET CONSOLE IN FNC.
	CALL	BDOS		;GET THE CHAR
CKCTLS:
	MVI		C,CONIN		;SET UP FOR SECOND READ
	CPI		13H		;TEST FOR CTL-S PAUSE
	CZ		BDOS		;IF SO, READ 2ND CHAR
CKBREAK:
	POP		H		;RESTORE ADDR
	CP		03H		;CHECK CHAR FOR ABORT
	JZ		EXIT		;EXIT IF SO
	CP		0DH		;IF CARRIAGE RETURN,
	JZ		EXIT		;ALSO ABORT
INCRSP:
	CPI		20H		;IF ITS A SPACE, BUMP H.
	RNZ			;H, O
	INR		H		;BUMP ADDR BY 100 (H BY 1)
	RET			;AND RETURN
;
NSTAT:
	INR		A		;NOT INTERRUPT SO JUST
	RET			;	RETURN A =1
;
	DS	100		;STACK SPACE
STACK:	EQU $
